#!/usr/bin/env python3
# With the shebang line you can run the script as ./script.py
# /env in the end grants portability

import copy
import json
import logging
import operator  # Set of functions corresponding to intrinsic operators
import os
import re
import sys  # System specific parameters
from collections import (OrderedDict,
                         namedtuple)
from functools import (partial,  # Returns partial of a function
                       reduce)  # For performing computation on a list
from typing import (Optional,  # Optional[X] is equiv. to Union[X, None]
                    Callable,  # Instance with a __call__ method
                    ...
                    Tuple,
                    List, 
                    IO)  # Wrapper namespace for I/O stream types
...
# Compiles RegExp pattern into RegExp object
# r - raw string literals
# (?<=regex) - positive lookbehind, looking for regex
# \b - word boundary; \s - whitespace
# (?P<name>regex) - named group
# \s* - 0 or more whitespaces; \w+ - 1 or more words
MODULE_USE_RE = re.compile(
    r'(?<=\buse\s)(?P<module>\s*\w+)',
    re.IGNORECASE)

# looking for word after 'module' except 'procedure'
# 'module procedure' is a part of interface declar-n, we don't need it
MODULE_DEFINITION_RE = re.compile(
    r'(?<=\bmodule\s)(?!\s*procedure)(\s*\w+)',
    re.IGNORECASE)

# Looking for everything inside quotes
# '|'.join(sequence) - joins into a string with | as separator - OR
# [^\'] - match everything except quote sign
...
# Makes it a callable script
@click.group()
def main() -> None:
    # Changing default logging level from WARNING to DEBUG
    logging.basicConfig(level=logging.DEBUG)


# Makes following function a command as arg in command line (CL)
@main.command()
@click.option('--path', '-p',  # CL 'run' args to provide a path
              default=os.getcwd(),  # Current working dir
              # TODO: Make a Pull Request with this
              type=click.Path(exists=True),  # Arg type, check if exists
...
def parse_modules_names_by_modules_paths(
        path: str,  # Path with all the folders including Fortran files
        # can be: parse_defined_modules_names - for defined modules
        # or: parse_used_modules_names - for used modules / includes
